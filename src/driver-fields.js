...
export const TREE_STATUS_FIELDS = ['lastStatus','lastReason','lastNote','lastDriver','lastReportAt','history','datum','zustand','lastFuellgrad','runStatus'];
...
export function onlyTreeStatusFields(obj){
  const out = {};
  // k.split('.')[0]: erlaubt auch verschachtelte Schreibpfade wie `runStatus.<tourId>` (Pro-Tour-Status)
  for(const k of Object.keys(obj || {})) if(TREE_STATUS_FIELDS.includes(k.split('.')[0])) out[k] = obj[k];
  return out;
}
