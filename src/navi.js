...
import { installErrorHandler } from './errlog.js'; installErrorHandler('navi');
  ...
  try{
    ...
    projSnap.docs.forEach((p,i)=>{
      tourSnaps[i].forEach(t=>{
        const td=t.data();
        if(td.uebersicht) return; // Übersichtstouren sind keine echten Touren → nicht in der Navi-App
