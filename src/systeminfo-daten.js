...
export const SI_DIENSTE = [
  { gruppe: 'Programmbibliotheken (im Browser)', items: [
    { name: 'Leaflet (lokal ausgeliefert)', zweck: 'Kartendarstellung in allen Apps', lizenz: 'BSD-2-Clause (Open Source)', frei: 'Ja, dauerhaft – auch kommerziell', status: 'ok', nachweis: 'https://github.com/Leaflet/Leaflet/blob/main/LICENSE', hinweis: 'Frei nutzbar, keine Lizenzkosten; Copyright-Hinweis im Quellcode. Seit 21.07.2026 samt Marker-Clustering und Rotations-Erweiterung mit der App selbst ausgeliefert (/vendor, prüfsummen-verifizierte Originale) — kein Nachladen von unpkg.com mehr: robust gegen blockierende Firmen-Proxys und keine IP-Übertragung an den CDN-Betreiber.' },
    { name: 'Chart.js (lokal ausgeliefert)', zweck: 'Diagramme (Controlling, Einsatzleiter)', lizenz: 'MIT (Open Source)', frei: 'Ja, dauerhaft – auch kommerziell', status: 'ok', nachweis: 'https://github.com/chartjs/Chart.js/blob/master/LICENSE.md', hinweis: 'Frei nutzbar, keine Auflagen. Seit 21.07.2026 lokal unter /vendor/lib (kein CDN-Nachladen mehr).' },
    { name: 'SheetJS (Community, lokal ausgeliefert)', zweck: 'Excel-Import', lizenz: 'Apache-2.0 (Open Source)', frei: 'Ja – Community-Edition kommerziell frei', status: 'ok', nachweis: 'https://docs.sheetjs.com/docs/miscellany/license/', hinweis: 'Die genutzte Community-Edition ist frei; nur die optionale „Pro"-Variante kostet. Seit 21.07.2026 lokal unter /vendor/lib.' },
    { name: 'html-to-image (lokal ausgeliefert)', zweck: 'Karten-/Berichts-Ausdruck (Rasterung)', lizenz: 'MIT (Open Source)', frei: 'Ja, dauerhaft – auch kommerziell', status: 'ok', nachweis: 'https://github.com/bubkoo/html-to-image/blob/master/LICENSE', hinweis: 'Seit 21.07.2026 lokal unter /vendor/lib (kein CDN-Nachladen mehr).' },
    { name: 'proj4js (lokal ausgeliefert)', zweck: 'Koordinaten-Umrechnung bei Import und Shapefile-Export (Gauß-Krüger/UTM ↔ WGS84)', lizenz: 'MIT (Open Source)', frei: 'Ja, dauerhaft – auch kommerziell', status: 'ok', nachweis: 'https://github.com/proj4js/proj4js/blob/master/LICENSE.md', hinweis: 'Reine Rechen-Bibliothek im Browser, keine externen Aufrufe/Datenübertragung. Seit 21.07.2026 lokal unter /vendor/lib. Beim Excel-Import: alte Katasterkoordinaten (Gauß-Krüger, EPSG:31466/67) → WGS84. Beim Shapefile-Export: WGS84 → ETRS89/UTM 32N (EPSG:25832).' },
    { name: 'Firebase JS SDK', zweck: 'Verbindung zur Datenbank/Anmeldung', lizenz: 'Apache-2.0 (Client-Bibliothek)', frei: 'Ja – Kosten nur über die Dienste (s. u.)', status: 'ok', nachweis: 'https://github.com/firebase/firebase-js-sdk/blob/master/LICENSE', hinweis: 'Die Bibliothek ist frei; Kosten entstehen nur durch die Nutzung der Google-Dienste.' },
  ]},
  { gruppe: 'Karten & Geodaten', items: [
    { name: 'basemap.de (amtliche Standardkarte — Farbe + Graustufen)', zweck: 'Standard-Hintergrundkarte in allen Apps', lizenz: 'CC BY 4.0 (BKG / AdV)', frei: 'Ja – auch kommerziell/kommunal, mit Quellenangabe', status: 'ok', nachweis: 'https://sgx.geodatenzentrum.de/web_public/gdz/lizenz/deu/nutzungsbedingungen_basemapde.pdf', hinweis: 'Amtliche Karte der deutschen Vermessungsverwaltung (Bundesamt für Kartographie und Geodäsie). Kostenfrei, DSGVO-konform (deutsche Behördenserver), bundesweit einheitlich. Ersetzt seit 15.06.2026 den OSM-Kachelserver UND das Esri-Satellitenbild (beide waren rechtlich riskant).' },
    { name: 'Amtliche Luftbilder/WMS (Landes-DOP, je Stadt)', zweck: 'Scharfes Luftbild (20 cm) je Projekt — als „Luftbild" wählbar', lizenz: 'dl-de/by-2-0 bzw. dl-de/zero-2-0 / CC BY 4.0 (Landesvermessung)', frei: 'Ja – auch kommerziell, mit Quellenangabe', status: 'ok', nachweis: 'https://www.govdata.de/dl-de/by-2-0', hinweis: 'Amtliche Geobasisdaten der Landesvermessung — rechtlich sauberste, schärfste Luftbildquelle. Nachweis-Link zeigt den Lizenztext „Datenlizenz Deutschland – Namensnennung 2.0"; die konkrete Lizenz des jeweiligen Landesdienstes steht in dessen WMS-GetCapabilities. Eingebunden: RLP (Ludwigshafen), Hessen (Rüsselsheim), NRW (Rheine, Ahlen, Bonn — inkl. transparentem Beschriftungs-Overlay mit Straßennamen, Dienst wms_nw_dop_overlay), Niedersachsen (Bad Rothenfelde, Wilhelmshaven). Hinweis: Ein freies transparentes Straßennamen-Overlay gibt es bisher nur in NRW; Hessen/Niedersachsen bieten dafür keinen kostenfreien Dienst. Neue Städte: passenden Landesdienst unter WMS-Karten ergänzen.' },
    { name: 'OpenRouteService (Routing inkl. Optimierung)', zweck: 'Tour-Fahrstrecken + Reihenfolge-Optimierung (Matrix bis 50 Stopps, ab 51 der Optimierungs-Endpunkt /optimization) sowie Routenlinie und Navigation in der Fahrer-App', lizenz: 'Dienst von HeiGIT; Daten OSM/ODbL', frei: 'Kostenloser Plan mit Tageslimit (ca. 2.000 Routen/Tag, 500 Matrix/Tag, 500 Optimierungen/Tag) + Job-Limit je Optimierungs-Anfrage', status: 'achtung', nachweis: 'https://openrouteservice.org/plans/', hinweis: 'WICHTIG: Der öffentliche ORS-Schlüssel ist gratis, aber „Fair-Use" — laut Nutzungsbedingungen NICHT für kommerziellen Dauerbetrieb gedacht und ohne Verfügbarkeitsgarantie. Je Stadt eigener API-Schlüssel; Navigation (Turn-by-turn) und Optimierung erhöhen das Volumen. VOR produktivem/breitem Rollout: eigenen ORS-Server selbst hosten (Open Source, eigene OSM-Daten → keine Fremd-ToS, kein Tages-/Job-Limit, nur Serverkosten) ODER kostenpflichtigen Tarif buchen. OSM-Namensnennung bleibt. Der frühere öffentliche OSRM-Demo-Server wird nicht mehr genutzt. Kosten (grobe Schätzung): gehosteter Pay-per-Call-Dienst ~0,50–5 €/1.000 Routen bzw. ~5–15 €/1.000 Optimierungen (oft Gratis-Kontingent) — Kostentreiber ist die Live-Navigation, nicht die Routenplanung (nur Planung meist gratis/wenige €; mit Navigation über viele Fahrer/Städte schnell drei-/vierstellig €/Monat). Self-Host ~20–60 €/Monat pauschal, unbegrenzt. Routing im Code gekapselt → Anbieterwechsel ohne großen Umbau.' },
    { name: 'Nominatim (Adress-/Straßensuche)', zweck: 'Adress-Suchfeld auf der Karte + Betriebshof-Adresse → Koordinaten', lizenz: 'Adressdaten ODbL (OSM); OSMF-Nutzungsrichtlinie', frei: 'Ja, aber nur Einzelsuchen (kein Tippen-Autocomplete), max. 1/Sek., mit Quellenangabe', status: 'achtung', nachweis: 'https://operations.osmfoundation.org/policies/nominatim/', hinweis: 'Das Suchfeld sucht nur bei Enter (eine Anfrage pro Suche) und ist auf Deutschland/Stadt eingegrenzt — damit policy-konform. Quellenangabe ist eingebunden. Für intensive Nutzung oder amtliche Genauigkeit kann der Dienst gegen den BKG-Geokodierungsdienst (für Kommunen kostenfrei, Zugang beim BKG/ZSGT beantragen) getauscht werden — die Suchfunktion ist dafür gekapselt.' },  ]},
  { gruppe: 'Plattform & Sicherheit', items: [
    { name: 'Google Firebase / Cloud (Datenbank, Hosting, Speicher, Anmeldung, Funktionen, KI)', zweck: 'Gesamtes Backend', lizenz: 'Kommerzieller Google-Cloud-Vertrag (Pay-as-you-go)', frei: 'Großzügiges Gratis-Kontingent, danach nutzungsbasiert', status: 'ok', nachweis: 'https://firebase.google.com/pricing', hinweis: 'Rechtlich sauber (kommerzieller Anbieter, EU-Region Frankfurt). Für DSGVO Auftragsverarbeitungsvertrag mit Google abschließen (s. DSGVO-Checkliste).' },
    { name: 'Google reCAPTCHA v3 (App Check)', zweck: 'Schutz vor fremden Clients', lizenz: 'Google-Nutzungsbedingungen', frei: 'Praktisch kostenfrei — euer Volumen (wenige Clients) liegt weit unter jeder kostenpflichtigen Schwelle', status: 'achtung', nachweis: 'https://docs.cloud.google.com/recaptcha/docs/billing-information', hinweis: 'Ihr nutzt den KLASSISCHEN reCAPTCHA-v3-Anbieter (App-Check-Site-Key), nicht reCAPTCHA Enterprise. Firebase weist für reCAPTCHA v3 über App Check keinen eigenen Preis aus — für euch faktisch 0 €. Zur Einordnung der Google-Preise: seit der Umstellung 01.04.2024 sind bei „reCAPTCHA" (vormals Enterprise) 10.000 Prüfungen/Monat kostenfrei, danach ca. 8 $/Monat bis 100.000 bzw. ca. 1 $/1.000 darüber (früher 1 Mio gratis). Euer reales Volumen (nur App-Starts/Logins weniger Nutzer, Token-Autorefresh ~1 h) liegt klar unter 10.000/Monat → real 0 €. WATCH-ITEM: Google konsolidiert reCAPTCHA in Google Cloud und empfiehlt ausdrücklich den Umstieg auf reCAPTCHA Enterprise; klassische v3-Keys laufen langfristig aus. Vor breitem Rollout in der Firebase-Konsole (App Check) prüfen, ob der Key eine Migrationswarnung zeigt, und ggf. den App-Check-Anbieter auf „reCAPTCHA Enterprise" umstellen — bleibt bei eurem Volumen kostenfrei (10.000/Monat). Läuft unsichtbar; übermittelt technische Signale an Google (Datenschutzerklärung Ziffer 4).' },
  ]},
  { gruppe: 'Schriften', items: [
    { name: 'DM Sans / DM Mono (lokal ausgeliefert)', zweck: 'Schriftarten der Desktop-Oberfläche', lizenz: 'SIL Open Font License 1.1', frei: 'Ja – mit der App selbst ausgeliefert', status: 'ok', nachweis: 'https://fonts.google.com/specimen/DM+Sans/license', hinweis: 'DSGVO-konform: Schriften liegen lokal als woff2 unter /fonts und werden von der eigenen Domain geladen — keine Übertragung an Google, kein Nachladen vom Google-CDN (vgl. LG München 2022). Mobile-/Erfassungs-/Einsatzleiter-App nutzen System-Schriften.' },
  ]},
];
